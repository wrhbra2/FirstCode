...
while True:
        ...
        if valor == "1":
            ...
            df1 = {
                    ...
                    'Proventos': [float(input('Valor Dividendos: ').replace("0", "0"))],
                    'Compra': [float(input('Preço de Compra: R$  ').replace("0", "0"))],
                    'Venda': [float(input('Preço de Venda: R$  ').replace("0", "0"))]
                    }

            df1 = pd.DataFrame(df1, columns=['Data', 'Ativo', 'Classe', 'Quantidade', 'Proventos', 'Compra', 'Venda']);
                                
            #df1['Total'] = round((df1['Venda']* df1['Quantidade'])-(df1['Compra']*df1['Quantidade']), 2)
            #print(df1)
            #Criar lista de ativos como variavies.
            
            df1['Total'] = round(df1['Compra']* df1['Quantidade'],2) 

            lista = [df1['Ativo'],df1['Quantidade'],df1['Total']]
            print(lista)
            ...
            if file == 0:
                    ...
                    data=df3.groupby(['Ativo'])[['Quantidade', 'Proventos', 'Compra', 'Venda', 'Total']]
                    
                    #Preço Médio.
                    data['Preço_médio']=round(data['Total']/data['Quantidade'], 2)
                    
                    
                    #Preço alvo
                    data['Preço_Alvo']= round(data['Compra']*2,2)


                    # Patrimonio
                    data['Patrimonio']=round(data['Total'].sum(), 2)
                     

                    # Darf
                    PCA=28418.69  # Prejuizo compensar ações
                    # PCA = (float(input('Digite Prejuizo a Compensar Ações <$28418.69>'))

                    PCF=635.88   # Prejuizo a compensar FII
                    # PCF = (float(input('Digite Prejuizo a Compensar FII <$635.88>'))
                    
                    
                    data['darf']=round(((data['Venda']*data['Quantidade'])-(data['Compra']*data['Quantidade'])*0.15)-PCA,2)
                    
                                                
                    
                    data.to_csv('balanco.csv')
                    print('Balanço da  Carteira Efetuado. \n', data)

                    # grafico = somatorio.groupby(['Ativo']).Ativo.count().sort_values()[-5:].plot(kind='bar')
                    data.set_index(['Quantidade']).plot(kind='bar')
                    # adicionando o títuloplt.xlabel('NOME DO EIXO X')
                    plt.title('Carteira de Investimentos - Balanço')
                    plt.xlabel('Ativo')
                    plt.ylabel('Valores')
                    print(plt.show())
        ...
        if valor == "2":
                    ...
                    'Quantidade': [int(input('Nova Quantidade: '))],
                    'Debito': [float(input(' Novo Debito: R$  '))],
                    'Credito': [float(input(' Novo Crédito: R$  '))],
                    'Doações': [float(input(' Nova Doação: R$  '))],
                    'Aportes': [float(input(' Novo Aporte: R$  '))],
                    'Reserva': [float(input(' Nova Reserva: R$  '))],
                    'Mimos': [float(input(' Novo Mimo: R$  '))]
                     }
                    df1=pd.DataFrame(df1, columns=[
                                     'Data', 'Item', 'Quantidade', 'Debito', 'Credito', 'Doações', 'Aportes', 'Reserva', 'Mimos'])
                    df1.head()
                    ...
                    if file == 0:
                            ...
                            # Leitura de registros anteriores
                            df2=pd.read_csv('orçamento.csv', index_col=[0])
                            df2.head()
                            ...
                            # Fechamento do Mês.
                            df3=pd.read_csv('orçamento.csv', index_col=[
                                            0])  # Leitura Arquivo
                            df3['Data']=pd.to_datetime(
                                df3['Data'])  # Conversão Objetos
                            fecha_mes=df3.groupby(df3['Data'].dt.month)[
                                                  ['Debito', 'Credito', 'Doações', 'Aportes', 'Reserva', 'Mimos']].apply(sum).reset_index()  # Soma das colunas
                            print('Balanço Orçamentario Efetuado. \n', fecha_mes)
                            fecha_mes.to_csv('balanço.csv')

                            # Graficos
                            plt.style.use('dark_background')
                            fecha_mes.plot.line(grid=True, color=['y', 'r'], x='Data', y=[
                                                'Credito', 'Debito'])
                            # adicionando o títuloplt.xlabel('NOME DO EIXO X')
                            plt.title('Orçamentos - Balanço')
                            plt.xlabel('Itens')
                            plt.ylabel('Valores')
                            print(plt.show())
        ...
        if valor == "3":
                 ...
                 if file == 0:
                    print('Documento não encontardo')
                     ...
                     if vendas < 20000:
                             print(
                                 'Você esta isento de pagar DARF sobre o lucro das vendas.')

                     else:
                         for lista in df1['Classe']:
                             # variável prejuiizo a compensar açoes.
                             valor=28418.69
                         for fii in df1['Classe']:
                             valor=635.88  # variável prejuizo compensar fii.
                             df1['DARF']=round(
                                 (df1['Total'] - valor) * 0.15, 2)
                             df1.to_csv('carteira.csv')
        ...
        if valor == "4":
            ...
            # Período máximo
             ...
             # Datas específicas
             mxrf.history(start='2020-12-10', end='2021-01-31')
        ...
        continuar=input(
            "Deseja realizar outra operação? Digite S para SIM ou N para NÃO. ")
